...
export function pressKey(): Promise<string> {
  return new Promise((resolve) => {
    rl.question('\nPress enter to continue or leave if you are scared.\n', (userInput) => {
      if (userInput.trim() === "") {
        resolve(userInput);
      } else {
        console.log('You must only press Enter to continue.');
        pressKey().then(resolve); 
      }
    });
  });
}
    ...
    rl.question(
      ...
      async (answer) => {
        if (answer === "1") {
          ...
          console.log('You have to press "1" or "2", if you press another key you cannot advance in your quest.😤');
